...
test('pick the longest size', (t) => {
  ...
  t.equal(pav.pickLongestSize(null, null, 'def123'), 'def123', 'null')
  t.equal(pav.pickLongestSize([null, '']), '', 'discard null')
  t.equal(pav.pickLongestSize(['ab','a','abcd'], 2),'ab', 'with max')
...
test('pick the shortest size', (t) => {
  ...
  t.equal(pav.pickShortestSize([], null, 'def123'), 'def123', 'empty')
  t.equal(pav.pickShortestSize([null, '', 'abc']),'', 'discard null')
  t.equal(pav.pickShortestSize(['ab','a','abcd']),'a', 'A')
  t.equal(pav.pickShortestSize(['ab','a','abcd'], 2),'ab', 'with min')
